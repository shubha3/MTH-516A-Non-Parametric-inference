...
for (i in 1:1000)
{
data1=as.data.frame(f1(100,0,beta))
a <- lmrob(Y~X2,data=data1,k.max=5000)
foo <- frb(lmrob.object=a, nboot=10000,return.coef=TRUE,return.indices=TRUE)
cc=cov(foo$coef)
...
for (i in 1:1000)
{
data2=as.data.frame(f1(100,0,beta))
...
cbind(len1/1000,covr1)
endtime3 <- Sys.time()
print(endtime3-starttime3)
plot(data1$X2,data1$Y,main="simulated analysis with zero contamination")
abline(a$coef[1],a$coef[2])
abline(lm(Y~X2,data=data1),col="red")
abline(5,5,col="blue")
legend(-2, 20, legend=c("OLS","MM","TRUE model"), fill = c("red","black","blue"))